...
export class ProdutosController {
    ...
    @ApiOperation({
        ...
        description: 'Disponível apenas para Admin. Adicione a imagem com a chave: files.',
      })
    @ApiResponse({ status: 204})  
    ...
    @ApiOperation({
        ...
        description: 'Disponível apenas para Admin. Atualização da imagem é opcional. Adicione a imagem com a chave: files.',
      })
    ...
    @CheckPolicies((ability: AppAbility) => ability.can(Action.Admin, 'all'))
    @Put(':id')
    update(@Param('id') id: string, @Body() body: ProductsDto, @UploadedFiles() files: Express.Multer.File[]){
